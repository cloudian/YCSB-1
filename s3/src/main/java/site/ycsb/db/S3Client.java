 ...
 * Properties to set:
 *
 * s3.accessKeyId=access key S3 aws s3.secretKey=secret key S3 aws
 * s3.endPoint=s3.amazonaws.com s3.region=us-east-1 The parameter table is the
 * name of the Bucket where to upload the files. This must be created before to
 * start the benchmark The size of the file to upload is determined by two
 * parameters: - fieldcount this is the number of fields of a record in YCSB -
 * fieldlength this is the size in bytes of a single field in the record
 * together these two parameters define the size of the file to upload, the size
 * in bytes is given by the fieldlength multiplied by the fieldcount. The name
 * of the file is determined by the parameter key. This key is automatically
 * generated by YCSB.
 *
 */
public class S3Client extends DB {
   ...
   * @return OK on success, otherwise ERROR. See the {@link DB} class's
  ...
  /**
   * Initialize any state for the storage. Called once per S3 instance; If the
   * client is not null it is re-used.
   */
  @Override
    ...
    synchronized (S3Client.class) {
      ...
      if (count <= numberOfOperations) {
        ...
        try {
          ...
          // Getting the values for Range Reads
          String rangeReadEnabledString = props.getProperty("s3.rangeReadEnabled");
          if (rangeReadEnabledString == null) {
            rangeReadEnabledString = propsCL.getProperty("s3.rangeReadEnabled", "false");
          }
          ...
          if (rangeReadSizeString == null) {
            rangeReadSizeString = propsCL.getProperty("s3.rangeReadSize", "0");
          }
   ...
   * values HashMap will be written into the file with the specified record key.
   *
   * @param bucket The name of the bucket
   * @param key    The record key of the file to insert.
   * @param values A HashMap of field/value pairs to insert in the file. Only the
   *               content of the first field is written to a byteArray multiplied
   *               by the number of field. In this way the size of the file to
   *               upload is determined by the fieldlength and fieldcount
   *               parameters.
   * @return OK on success, ERROR otherwise. See the {@link DB} class's
   *         description for a discussion of error codes.
   */
  @Override
  ...
  /**
   * Read a file from the Bucket. Each field/value pair from the result will be
   * stored in a HashMap.
   *
   * @param bucket The name of the bucket
   * @param key    The record key of the file to read.
   * @param fields The list of fields to read, or null for all of them, it is null
   *               by default
  ...
  /**
   * Update a file in the database. Any field/value pairs in the specified values
   * HashMap will be written into the file with the specified file key,
  ...
  /**
   * Perform a range scan for a set of files in the bucket. Each field/value pair
   ...
   *                    field/value pairs for one file
   * @return OK on success, ERROR otherwise.
   */
  @Override
   ...
   * @param updateMarker A boolean value. If true a new object will be uploaded to
   *                     S3. If false an existing object will be re-uploaded
   *
   */
  protected Status writeToStorage(String bucket, String key, Map<String, ByteIterator> values, Boolean updateMarker,
  ...
  private S3Object getS3ObjectAndMetadata(String bucket, String key, SSECustomerKey ssecLocal) {
